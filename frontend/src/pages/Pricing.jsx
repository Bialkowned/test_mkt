    ...
    <div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          <div className="flex gap-8">
            ...
            <div className="flex-1 space-y-5">
                ...
                <div key={role.id} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  ...
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center gap-3">
                      <span className="w-7 h-7 rounded-full bg-gray-900 text-white flex items-center justify-center text-xs font-bold">{ri + 1}</span>
                      <input
                        type="text"
                        placeholder="Role name (e.g. Admin)"
                        className="px-3 py-2 border border-gray-200 rounded-lg text-sm font-semibold focus:ring-2 focus:ring-primary-500 focus:border-primary-500 w-56"
                        value={role.name}
                        onChange={(e) => updateRoleName(role.id, e.target.value)}
                      />
                    </div>
                    {roles.length > 1 && (
                      <button onClick={() => removeRole(role.id)} className="text-xs text-gray-400 hover:text-red-500 font-medium">Remove role</button>
                    )}
                  </div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {SERVICE_TYPES.map((st) => (
                              <button
                                key={st.id}
                                type="button"
                                onClick={() => updateItem(role.id, item.id, 'service', st.id)}
                                className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                  item.service === st.id ? st.pill : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'
                                }`}
                              >
                                {st.name}
                              </button>
                  ...
                  <button onClick={() => addItem(role.id)} className="mt-4 text-sm text-primary-600 hover:text-primary-700 font-medium">
                    + Add item
                  </button>
                </div>
              ))}
              ...
              <button
                onClick={addRole}
                className="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-sm text-gray-500 hover:border-gray-400 hover:text-gray-700 font-medium transition-colors"
              >
                + Add Role
              </button>
            </div>
            ...
            <div className="w-72 shrink-0 hidden md:block">
              <div className="bg-white border border-gray-200 rounded-xl p-6 sticky top-24 shadow-sm">
                <h3 className="text-sm font-bold text-gray-900 mb-4">Cost Preview</h3>
                    ...
                    return (
                      <div key={r.id} className="pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                        <div className="flex justify-between text-sm">
                          <span className="font-medium text-gray-700 truncate">{r.name || 'Unnamed'}</span>
                        ...
                        <div className="flex gap-3 text-xs text-gray-400 mt-0.5">
                          ...
                          {rolePages > 0 && <span>{rolePages} pg</span>}
                        </div>
                      </div>
                    )
                  })}
                ...
                <p className="text-xs text-gray-400 mt-4 leading-relaxed">Testers can accept or counter-bid your prices. You only pay when accepting a bid.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-14">
            {[
              ...
              <div key={item.step} className="text-center">
                <div className="w-10 h-10 rounded-full bg-primary-600 text-white flex items-center justify-center text-lg font-bold mx-auto mb-4">
                  {item.step}
                </div>
      ...
      <section className="bg-gray-900">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          <p className="text-gray-400 mb-8">Create structured test jobs with roles, items, and service types. Testers bid for your work.</p>
          {user ? (
            <Link
              to="/jobs/create"
